...
try {
    // Step 1: Connect to MySQL server without a database name first to create it if needed
    ...
    // Automatically create required tables if they are missing
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS finances (
        ...
        type ENUM('income', 'fee', 'debt') NOT NULL,
        amount DECIMAL(10,2) NOT NULL,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    )");

    // Added reward_history table to track point changes for users
...
<title>Kahawa Club - Admin & Cashier Panel</title>
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; }
  ...
  .back-link { display: block; text-align: center; font-size: 12px; color: #666; text-decoration: none; margin-top: 6px; font-weight: bold; }
  .back-link:hover { color: #111; }
  /* Graphical Sales Chart Styles */
...
<div class="app-container">
  ...
  <div id="audio-banner" style="background: #2d6a4f; color: white; padding: 10px; border-radius: 12px; text-align: center; margin-bottom: 16px; font-size: 11px; font-weight: bold; display: flex; justify-content: space-between; align-items: center;">
    <span>🔔 Enable audio alerts</span>
    <button onclick="enableSound()" style="background: #fff; color: #2d6a4f; border: none; padding: 6px 12px; border-radius: 8px; font-weight: bold; cursor: pointer; font-size: 10px;">Enable Sound</button>
  </div>
  <div class="header">
  ...
  <a href="dashboard.php" class="back-link">&larr; Return to Customer Dashboard</a>
  <!-- Graphical Sales Analytics Chart -->
...
<script>
    let audioContextUnlocked = sessionStorage.getItem('soundUnlocked') === 'true';
    if (audioContextUnlocked) {
        try {
            const AudioContext = window.AudioContext || window.webkitAudioContext;
            window.myAudioCtx = new AudioContext();
        } catch (e) {
            console.log("Auto-init error:", e);
        }
    }
    function enableSound() {
        try {
            ...
            banner.innerHTML = '<span>🔊 Bell alerts active and loud!</span>';
        } catch (e) {
    ...
    function playBeep() {
        ...
        try {
            ...
            const bellStrikes = [0, 1.5, 3.0, 4.5];
            const baseFreq = 440.0;
            bellStrikes.forEach(strikeTime => {
                let startTime = ctx.currentTime + strikeTime;
                const harmonics = [
                    { ratio: 1.0, gain: 0.6, decay: 1.4 },
                    { ratio: 2.76, gain: 0.3, decay: 1.0 },
                    { ratio: 5.4, gain: 0.15, decay: 0.7 }
                ];
                harmonics.forEach(h => {
                    let osc = ctx.createOscillator();
                    let gain = ctx.createGain();
                    osc.type = 'sine';
                    osc.frequency.setValueAtTime(baseFreq * h.ratio, startTime);
                    gain.gain.setValueAtTime(0.01, startTime);
                    gain.gain.linearRampToValueAtTime(h.gain, startTime + 0.03);
    ...
    let lastOrderCount = <?php echo intval($totalOrders); ?>;
    setInterval(function() {
        fetch('check_new_orders.php')
        .then(response => response.json())
        .then(data => {
            if (data.orderCount && data.orderCount > lastOrderCount) {
                playBeep();
                setTimeout(() => {
                    location.reload();
                }, 6000);
